<?php 
	...
	if (isset($_POST['id'])) {
		$id=$_POST['id'];
		$placeholder=$PDO->query("SELECT * FROM professeur WHERE idprof='$id'");
	...
	{	
		...
		$grade=$_POST['grade'];
		$req = $PDO->prepare('UPDATE professeur SET nom = :nom,prenoms = :prenom,civilite = :civilite,grade = :grade WHERE idprof = :id');
		$req->execute(array(
				'nom' => $nom,
				'prenom' => $prenom,
				'civilite' => $civilite,
...
<body>
	<form method="POST" action="maj_prof.php">
		<fieldset>
			...
			<div>	
				<label>ID ;</label>
				<input type="text" name="id" value="<?php if (isset($placeholder['idprof'])) { echo $placeholder['idprof'];}  ?>" required readonly>
			</div>
			<div>
				<label>Nom :</label>
				<input type="text" name="nom" value="<?php if (isset($placeholder['nom'])) { echo $placeholder['nom'];}  ?>" required>	
			</div>
			<div>
				<label>Prénoms :</label>
				<input type="text" name="prenom" value="<?php if (isset($placeholder['prenoms'])) { echo $placeholder['prenoms'];}  ?>" required>	
			</div>
			<section>
				<p>Civilité : <br>
