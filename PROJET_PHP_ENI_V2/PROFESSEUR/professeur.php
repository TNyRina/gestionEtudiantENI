<?php 
	...
	if (isset($_POST['btn_rechercher'])) {
		if (isset($_POST['recherche'])) {
			...
			$req = $PDO->query("SELECT * FROM professeur WHERE idprof='$chercher' or nom like '%$chercher%'");
			$req = $req->fetchall(PDO::FETCH_ASSOC);
			if ($req==null) {
				$message_recherche = $chercher." introuvable!";
			}
			$conter = $PDO->query("SELECT count(*) as nb FROM professeur WHERE idprof='$chercher' or nom like '%$chercher%'" );
	...
	elseif (isset($_POST['btn_supprimer'])) {
		try {
			$id = $_POST['id'];
			$req = $PDO->query("DELETE FROM professeur WHERE idprof='$id'");
				...
				foreach ($req as $value) {
					...
					<span class=\"btn\">
						<form action=\"maj_prof.php\" method=\"POST\">
							<input type=\"hidden\" name=\"id\" value=\"".$value['idprof']."\">
